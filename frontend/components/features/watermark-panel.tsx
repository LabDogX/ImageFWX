...
function FontSizeControl({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  const { t } = useLocale();
  const update = (next: number) => onChange(Math.max(8, Math.min(128, Math.round(next))));

  return <label className="col-span-2 text-xs">
    <span className="flex items-center justify-between"><span>{t('Size')}</span><span>{value}px</span></span>
    <div className="mt-1 flex items-center gap-2">
      <Slider className="flex-1" value={[value]} min={8} max={128} step={1} onValueChange={([next]) => update(next)} />
      <Input className="w-20" type="number" min="8" max="128" step="1" value={value} onChange={event => update(Number(event.target.value) || 8)} aria-label={t('Size')} />
  ...
  return <div className="space-y-2 rounded border p-3">
    ...
    {value.enabled && <>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <FontSizeControl value={value.font_size} onChange={font_size => onChange({ font_size })} />
...
export function WatermarkPanel({ value, onChange, libraryImages, currentImageId }: { value: WatermarkStack; onChange: (value: WatermarkStack) => void; libraryImages: LibraryImage[]; currentImageId: number }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-2 rounded border p-3"><label className="flex items-center justify-between text-sm font-medium">{t('Camera EXIF')}</label><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={value.exif.enabled} onChange={event => updateExif({ enabled: event.target.checked })} /> {t('Show shooting settings')}</label>{value.exif.enabled && <><div className="grid grid-cols-2 gap-2"><PositionSelect value={value.exif.position} onChange={position => updateExif({ position })} /><label className="text-xs">{t('Font')}<select className="mt-1 w-full rounded border bg-background p-2" value={value.exif.font} onChange={event => updateExif({ font: event.target.value })}>{fonts.map(([id, label]) => <option key={id} value={id}>{t(label)}</option>)}</select></label><label className="text-xs">{t('Separator')}<Input className="mt-1" maxLength={8} value={value.exif.separator} onChange={event => updateExif({ separator: event.target.value || ' · ' })} /></label><FontSizeControl value={value.exif.font_size} onChange={font_size => updateExif({ font_size })} /><OpacityControl value={value.exif.opacity} onChange={opacity => updateExif({ opacity })} /></div><div className="flex flex-wrap gap-x-3 gap-y-1">{exifFields.map(field => <label key={field} className="text-xs"><input className="mr-1" type="checkbox" checked={value.exif.fields.includes(field)} onChange={event => updateExif({ fields: event.target.checked ? [...value.exif.fields, field] : value.exif.fields.filter(current => current !== field) })} />{t(field)}</label>)}</div><p className="text-xs text-muted-foreground">{t('GPS location is never read or displayed.')}</p></>}</div>
