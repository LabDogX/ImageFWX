...
const zhCN: Record<string, string> = {
  ...
  'My border templates': '我的边框模板', 'Frame style': '边框样式',
  'Solid color': '纯色', 'Gradient': '渐变色', 'Frosted glass': '毛玻璃',
  'Start color': '起始颜色', 'End color': '结束颜色', 'Angle': '角度',
  'Tint color': '着色颜色', 'Tint opacity': '着色不透明度',
  'Floating Paper': '悬浮相纸', 'Frosted Glass': '毛玻璃边框',
  'Sunset Gradient': '日落渐变', 'Ocean Gradient': '海洋渐变',
  ...
  'Discard': '放弃修改',
  'Filename updated': '文件名已更新', 'Failed to rename file': '重命名文件失败',
  'Applying crop...': '正在应用裁剪…', 'Crop applied!': '裁剪已应用！', 'Crop failed': '裁剪失败',
  'Changes saved': '修改已保存', 'Saving changes...': '正在保存修改…', 'Changes saved!': '修改已保存！',
  'Save failed': '保存失败', 'Reset to defaults': '已恢复默认设置',
  'Processing and downloading...': '正在处理并下载…', 'Downloaded!': '下载完成！',
  'Removing background with AI... This may take 30-60 seconds': '正在使用 AI 移除背景，可能需要 30–60 秒',
  'Background removed!': '背景已移除！', 'No result returned': '未返回处理结果',
  'Unknown error': '未知错误', 'AI service unavailable': 'AI 服务不可用',
  'Operation timed out': '操作超时', 'Try with a smaller image': '请尝试更小的图片',
  'Failed to remove background': '移除背景失败', 'Enhancing...': '正在增强…',
  'Image enhanced!': '图片已增强！', 'Enhancement failed': '增强失败',
  'Upscaling {scale}x... This may take a moment': '正在放大 {scale} 倍，可能需要一点时间',
  'Upscaled to {width}x{height}!': '已放大至 {width}×{height}！',
  'Upscale failed': '放大失败', 'Server error': '服务器错误', 'Failed to upscale image': '放大图片失败',
  'Unsaved changes will be lost. Close anyway?': '未保存的修改将会丢失，仍要关闭吗？',
  'Vintage': '复古', 'Hi-Contrast': '高对比度', 'Bright': '明亮', 'Dark Filter': '暗调',
  'Social': '社交媒体', 'Web': '网页', 'Avatar': '头像', 'Thumb': '缩略图',
  'Original: {width} × {height} px': '原始：{width} × {height} 像素',
  'New: {width} × {height} px': '新尺寸：{width} × {height} 像素',
  '{width} × {height}px selected': '已选择 {width} × {height} 像素',
  'Click and drag on the image to select crop area, then click "Apply Crop"': '在图片上点击并拖动以选择裁剪区域，然后点击“应用裁剪”',
  'Enter watermark...': '输入水印文字…', 'Font': '字体',
  'My watermark templates': '我的水印模板', 'Template name': '模板名称',
  'Logo': '徽标', 'Primary text': '主文字', 'Secondary text': '次要文字',
  'Camera EXIF': '相机 EXIF 参数', 'Show shooting settings': '显示拍摄参数',
  'Separator': '分隔符', 'GPS location is never read or displayed.': '不会读取或显示 GPS 位置信息。',
  'Layers are rendered in order: logo, primary text, secondary text, camera EXIF.': '图层按以下顺序渲染：徽标、主文字、次要文字、相机 EXIF 参数。',
  'Use an uploaded PNG, JPEG, WebP, or SVG as a logo/image watermark': '使用已上传的 PNG、JPEG、WebP 或 SVG 作为徽标/图片水印',
  'Select an uploaded image': '选择已上传图片', 'Scale: {count}% of short edge': '缩放：短边的 {count}%',
  'X offset': 'X 轴偏移', 'Image watermark': '图片水印',
  'Text watermark "{text}" at {position} ({opacity}%)': '文字水印“{text}”位于{position}（{opacity}%）',
  'Image watermark at {position} ({opacity}%)': '图片水印位于{position}（{opacity}%）',
  'northwest': '左上', 'north': '上方', 'northeast': '右上', 'west': '左侧', 'east': '右侧',
  'southwest': '左下', 'south': '下方', 'southeast': '右下',
  'Upscale': '放大', 'Increase image resolution': '提高图片分辨率',
  'Upscaling in progress...': '正在放大…', 'Uses high-quality Lanczos algorithm with sharpening': '使用高质量 Lanczos 锐化算法',
  'AI-powered background removal': 'AI 智能背景移除', 'Removing...': '正在移除…',
  'This may take 30-60 seconds...': '可能需要 30–60 秒…', 'May take 30-60 seconds': '可能需要 30–60 秒',
  'Improve colors & quality': '改善色彩与画质',
  'Select operations to see command preview': '选择操作以查看命令预览',
  'Job ID: {id}': '任务 ID：{id}',
  'Sans': '无衬线', 'Sans Bold': '无衬线粗体', 'Sans Condensed': '窄体无衬线',
  'Serif': '衬线', 'Serif Bold': '衬线粗体', 'Serif Italic': '衬线斜体',
  'Mono': '等宽', 'Mono Bold': '等宽粗体',
};
